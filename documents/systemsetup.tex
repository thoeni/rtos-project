...
Add the new Java to your shell environment's \texttt{\$PATH} so it takes priority over other Javas you might have installed (yes, this means that different JDK can coexist):
	\begin{verbatim}
		echo 'export PATH=/opt/java/64/jdk1.6.0_33/bin:$PATH' >> ~/.bashrc
	\end{verbatim}
...
By default I found on my system the latest version of the \texttt{gcc} compiler, which - at the time of this writing is the 4.6: this version presented some fatal errors which didn't let me complete the compilation. A solution has been to install the 4.4 version (as for JDK, two different versions can coexist on the same system).
	\begin{verbatim}
...
\definecolor{lightgray}{gray}{0.93}
  ...
  \vskip\baselineskip%
I put stress on this because I'm sure it can save you time, and prevent you from banging the desktop with your forehead.\\
The \texttt{-j8} option tells the compiler to launch 8 parallel threads, therefore if you are compiling on a multi-core architecture, this may save you quite a lot of time.
...
\end{itemize}
Last suggestion, as the AOSP website says, it's convenient enabling the \texttt{CCACHE} option, to save time during the following compilations.
